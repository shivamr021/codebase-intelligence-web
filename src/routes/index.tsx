...
import { api, API_DOCS_URL, type IngestResponse } from "@/lib/api";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Codebase Intelligence — Understand any GitHub repo in seconds" },
      {
        name: "description",
        content:
      ...
      { property: "og:title", content: "Codebase Intelligence" },
      { property: "og:description", content: "AI-powered codebase analysis for any GitHub repo." },
    ],
  }),
  component: Index,
});

const FEATURE_TAGS = ["Q&A Chat", "Dependency Graph", "Bug Reports", "Arch Diagram"];
...
function Index() {
  ...
  const analyse = async (e: React.FormEvent) => {
    ...
    if (trimmed.length > 300) {
      setError("URL is too long. Please enter a valid GitHub repository URL.");
      return;
    }
    const GITHUB_RE = /^https:\/\/github\.com\/[\w.-]+\/[\w.-]+(\/.*)?$/;
    if (!GITHUB_RE.test(trimmed)) {
      setError("Please enter a valid public GitHub repository URL (https://github.com/owner/repo).");
      return;
    }
    setLoading(true);
    setError(null);
    setResult(null);
    try {
      const res = await api.ingest(trimmed);
      setResult(res);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground antialiased">
        ...
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-6">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold tracking-tight">
              Codebase<span className="text-muted-foreground">.Intel</span>
            </span>
          </div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="pointer-events-none absolute inset-0">
          ...
          <div
            ...
            style={{ background: "oklch(0.745 0.142 184 / 0.12)", filter: "blur(120px)" }}
          />
        ...
        <div className="relative mx-auto flex max-w-5xl flex-col items-center px-6 pb-24 pt-20 text-center md:pt-28">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-pretty text-base leading-relaxed text-muted-foreground md:text-lg">
            Paste a GitHub repo. We&apos;ll embed the code, map dependencies, surface bugs, and let
            you ask anything in plain English — <span className="text-foreground/90">with citations.</span>
          </p>

          {/* Input — dimensional, glowy */}
          <form onSubmit={analyse} className="group relative mx-auto mt-10 w-full max-w-2xl">
            {/* outer gradient halo */}
            <div
            ...
            <div className="mt-3 flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground/70">
              <span className="inline-flex items-center gap-2">
                <span>Press</span>
                <kbd className="rounded border border-white/10 bg-white/5 px-1.5 py-0.5 font-mono normal-case">Enter</kbd>
                <span>to index</span>
              </span>
            </div>
          </form>
          ...
          <div className="mt-16 flex flex-col items-center gap-3">
            ...
            <div className="flex flex-wrap items-center justify-center gap-x-7 gap-y-2 text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">
              {FEATURE_TAGS.map((tag, i) => (
                <span key={tag} className="flex items-center gap-7">
                  <span className="transition-colors hover:text-primary">{tag}</span>
        ...
        <section id="features" className="relative mx-auto max-w-7xl px-6 pb-24">
          <div className="mb-10 text-center">
            ...
            <h2 className="mt-3 text-3xl font-bold tracking-tight md:text-4xl">
              From <span className="font-mono text-primary">git</span> <span className="font-mono text-primary">clone</span> to deep understanding.
            </h2>
          </div>
      ...
      {/* RESULTS */}
      {result && (
        <section id="results" className="relative mx-auto max-w-7xl px-6 pb-24">
          <div className="mb-6 flex flex-wrap items-center gap-3 rounded-xl border border-white/10 bg-card/60 p-4 backdrop-blur">
            ...
            <div className="flex-1 min-w-0">
              <div className="truncate font-mono text-sm font-medium">{result.repo_name}</div>
              <div className="truncate text-xs text-muted-foreground">{result.message}</div>
            </div>
            <Stat label="Files" value={result.files_indexed} />
            <Stat label="Chunks" value={result.chunks_stored} />
            <Stat label="Graph" value={result.graph_ready ? "Ready" : "—"} />
          </div>

          <Tabs defaultValue="qa" className="w-full">
            <TabsList className="grid w-full grid-cols-2 bg-card/60 backdrop-blur md:grid-cols-4">
              <TabsTrigger value="qa" className="gap-2">
                <MessageSquareCode className="h-4 w-4" /> Q&A Chat
              </TabsTrigger>
              ...
              <TabsTrigger value="arch" className="gap-2">
            ...
            <div className="mt-6 rounded-2xl border border-white/10 bg-card/40 p-6 shadow-elevated backdrop-blur">
              <TabsContent value="qa" className="mt-0">
                <QATab repoName={result.repo_name} />
              </TabsContent>
              <TabsContent value="graph" className="mt-0">
                <GraphTab repoName={result.repo_name} />
              </TabsContent>
              <TabsContent value="bugs" className="mt-0">
                <BugReportTab repoName={result.repo_name} />
              </TabsContent>
              <TabsContent value="arch" className="mt-0">
                <ArchitectureTab repoName={result.repo_name} />
              </TabsContent>
            </div>
          </Tabs>
        </section>
